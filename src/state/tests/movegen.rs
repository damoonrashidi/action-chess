...
mod tests {
    ...
    fn king_on_empty_board() {
        ...
        let moves = gen.for_king(&Piece::King(White, Duration::from_millis(0)), &E5);
    ...
    fn king_cant_move_into_check() {
        ...
        board.set_piece_at(Some(Piece::Rook(Black, Duration::from_millis(0))), A1);
        let gen = MoveGen::new(&board);
        let moves = gen.for_king(&Piece::King(White, Duration::from_millis(0)), &B2);
    ...
    fn king_cant_move_into_pawn_check() {
        ...
        board.set_piece_at(Some(Piece::King(White, Duration::from_millis(0))), E5);
        let gen = MoveGen::new(&board);
        let moves = gen.for_king(&Piece::King(White, Duration::from_millis(0)), &E5);
    ...
    fn king_cant_move_into_opposing_king() {
        let mut board = Board::default();
        board.set_piece_at(Some(Piece::King(Black, Duration::from_millis(0))), D5);
        let gen = MoveGen::new(&board);
        let moves = gen.for_king(&Piece::King(White, Duration::from_millis(0)), &D3);
    ...
    fn king_castle_kingside() {
        let mut board = Board::default();
        board.set_piece_at(Some(Piece::Rook(White, Duration::from_millis(0))), H1);
        board.white_can_castle_kingside = true;
        let gen = MoveGen::new(&board);
        let moves = gen.for_king(&Piece::King(White, Duration::from_millis(0)), &E1);
    ...
    fn blocked_queenside_castle() {
        let mut board = Board::default();
        board.set_piece_at(Some(Piece::Queen(White, Duration::from_millis(0))), D1);
        board.set_piece_at(Some(Piece::Rook(White, Duration::from_millis(0))), A1);
        board.set_piece_at(Some(Piece::Rook(White, Duration::from_millis(0))), H1);
        board.white_can_castle_kingside = true;
    ...
    fn moved_king_castle_kingside() {
        ...
        board.white_can_castle_kingside = true;
        let gen = MoveGen::new(&board);
        let moves = gen.for_king(&Piece::King(White, Duration::from_millis(0)), &E2);
        assert!(!moves.contains(&Move::KingSideCastle(White)));
    }
    ...
    fn pinned_pawn() {
        ...
        let gen = MoveGen::new(&board);
        let moves = gen.for_pawn(&Piece::Pawn(White, Duration::from_millis(0)), &B6);
    ...
    fn pinned_rook() {
        ...
        let gen = MoveGen::new(&board);
        let moves = gen.for_rook(&Piece::Pawn(White, Duration::from_millis(0)), &B6);
    ...
    fn knight_on_standard_board() {
        let board = Board::new();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_knight(&Piece::Knight(White, Duration::from_millis(0)), &E4);
    ...
    fn knight_in_starting_position() {
        let board = Board::new();
        let gen = MoveGen::new(&board);
        let moves = gen.for_knight(&Piece::Knight(White, Duration::from_millis(0)), &G1);
    ...
    fn rook_on_empty_board() {
        ...
        let moves = movegen.for_rook(&Piece::Rook(White, Duration::from_millis(0)), &E4);
    ...
    fn rook_on_standard_board() {
        let board = Board::new();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_rook(&Piece::Rook(White, Duration::from_millis(0)), &A5);
    ...
    fn starting_rook() {
        let board = Board::new();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_rook(&Piece::Rook(White, Duration::from_millis(0)), &A1);
    ...
    fn infiltrated_starting_rook() {
        let board = Board::new();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_rook(&Piece::Rook(Black, Duration::from_millis(0)), &A1);
    ...
    fn bishop_on_empty() {
        let board = Board::default();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_bishop(&Piece::Bishop(White, Duration::from_millis(0)), &E5);
    ...
    fn all_black_starting_moves_with_capture() {
        ...
        board.set_piece_at(Some(Piece::Pawn(White, Duration::from_millis(0))), D6);
        let gen = MoveGen::new(&board);
        let moves = gen.get_possible_moves();
    ...
    fn white_pawn_at_start() {
        let board = Board::new();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_pawn(&Piece::Pawn(White, Duration::from_millis(0)), &A2);
    ...
    fn black_pawn_at_start() {
        let mut board = Board::new();
        board.set_piece_at(Some(Piece::Bishop(White, Duration::from_millis(0))), D5);
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_pawn(&Piece::Pawn(Black, Duration::from_millis(0)), &D7);
    ...
    fn pawn_capture() {
        let mut board = Board::default();
        board.set_piece_at(Some(Piece::Pawn(Black, Duration::from_millis(0))), D3);
        board.set_piece_at(Some(Piece::Bishop(White, Duration::from_millis(0))), C3);
        board.set_piece_at(Some(Piece::Rook(White, Duration::from_millis(0))), E3);
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_pawn(&Piece::Pawn(Black, Duration::from_millis(0)), &D4);
    ...
    fn black_pawn_at_start_with_capture() {
        let mut board = Board::default();
        board.set_piece_at(Some(Piece::Bishop(White, Duration::from_millis(0))), A6);
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_pawn(&Piece::Pawn(Black, Duration::from_millis(0)), &B7);
    ...
    fn promote_white_pawn() {
        let board = Board::default();
        let movegen = MoveGen::new(&board);
        let moves = movegen.for_pawn(&Piece::Pawn(Black, Duration::from_millis(0)), &B2);
        assert_eq!(moves.len(), 4);
    }
